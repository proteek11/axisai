#!/bin/bash
# Hotpatch: Fix learner chat 503 error in axis_message.py
# Root causes:
#   1. AIClient() called without required session_factory arg
#   2. ai_client.complete() called without required model arg
#   3. Response parsed with .get() instead of .choices[0].message.content

set -e
DEST="/home/axisai/axisai-backend/axis-ai/app/services/chat/axis_message.py"
BACKUP="${DEST}.bak.$(date +%s)"

echo "=== axis_message.py chat fix ==="
cp "$DEST" "$BACKUP"
...
echo "File written. Restarting axis-ai service..."
sudo systemctl restart axis-ai
sleep 3
sudo systemctl is-active axis-ai
echo "=== Done ==="
