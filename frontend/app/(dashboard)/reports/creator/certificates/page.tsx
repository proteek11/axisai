...
import { Award, RefreshCw, Download, Search } from 'lucide-react';
...
function CreatorCertificatesReport() {
  const [dateRange, setDateRange] = useState('this_month');
  const [search, setSearch] = useState('');

  const { data, isLoading, error, refetch } = useQuery<CertsResponse>({
    queryKey: ['reports', 'creator', 'certificates', dateRange],
    queryFn: () =>
      fetch(`/api/reports/creator/certificates?date_range=${dateRange}`)
        .then((r) => r.json()),
  });
  ...
  async function handleExport() {
    const res = await fetch('/api/reports/export/csv', {
      ...
      body: JSON.stringify({ report_type: 'creator_certificates', filters: { date_range: dateRange } }),
    });
    if (!res.ok) return;
    ...
    a.href = url;
    a.download = 'creator-certificates.csv';
    a.click();
  ...
  return (
    <div>
      <Header
        ...
        subtitle="Certificates earned by learners in your spaces"
        action={
          ...
          <div className="enterprise-card overflow-x-auto">
            {filtered.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-sm text-muted-foreground">No certificates issued yet in your spaces</p>
              </div>
            ) : (
              <>
...
export default function CreatorCertificatesPage() {
  return <CreatorCertificatesReport />;
}
