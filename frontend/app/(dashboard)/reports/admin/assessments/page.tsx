...
function AssessmentsReport() {
  const [dateRange, setDateRange] = useState('this_month');
  const [search, setSearch] = useState('');

  const { data, isLoading, error, refetch } = useQuery<AssessmentsResponse>({
    queryKey: ['reports', 'admin', 'assessments', dateRange],
    queryFn: () =>
      fetch(`/api/reports/admin/assessments?date_range=${dateRange}`)
        .then((r) => r.json()),
  });
  ...
  async function handleExport() {
    const res = await fetch('/api/reports/export/csv', {
      ...
      body: JSON.stringify({ report_type: 'admin_assessments', filters: { date_range: dateRange } }),
    });
    if (!res.ok) return;
  ...
  return (
    <div>
      <Header
        ...
        subtitle="Attempt counts, average scores and pass rates per assessment"
        action={
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    {['Assessment', 'Space', 'Attempts', 'Learners', 'Avg Score', 'Pass Rate'].map((h) => (
