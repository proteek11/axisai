...
async function handler(req: NextRequest, { params }: { params: { id: string; assessmentId: string } }) {
  const token = cookies().get('axis_access')?.value;
  if (!token) return NextResponse.json({ error: 'Unauthorised' }, { status: 401 });
  const url = `${BACKEND}/api/v1/spaces/${params.id}/assessments/${params.assessmentId}`;
  const opts: RequestInit = { method: req.method, headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, cache: 'no-store' };
  if (req.method === 'PATCH') opts.body = JSON.stringify(await req.json().catch(() => ({})));
