...
import { TrendingUp, RefreshCw, Download, BookOpen, Users } from 'lucide-react';

interface Space { space_id: string; title: string; }
interface SpacesListResponse { spaces: Space[]; }
...
function CreatorLearnerProgressReport() {
  const [selectedSpaceId, setSelectedSpaceId] = useState<string>('');

  const { data: spacesList } = useQuery<SpacesListResponse>({
    queryKey: ['reports', 'creator', 'spaces-list'],
    queryFn: () => fetch('/api/reports/creator/spaces').then((r) => r.json()),
  });

  const { data, isLoading, error, refetch } = useQuery<LearnerProgressResponse>({
    queryKey: ['reports', 'creator', 'learner-progress', selectedSpaceId],
    queryFn: () =>
      fetch(`/api/reports/creator/learner-progress/${selectedSpaceId}`)
        .then((r) => r.json()),
  ...
  async function handleExport() {
    const res = await fetch('/api/reports/export/csv', {
      ...
      body: JSON.stringify({ report_type: 'creator_learner_progress', filters: { space_id: selectedSpaceId } }),
    });
    if (!res.ok) return;
  ...
  return (
    <div>
      <Header
        ...
        subtitle="Item-level progress for each enrolled learner in a space"
        action={
          selectedSpaceId ? (
      ...
      <div className="p-6 space-y-6">
        ...
        {!selectedSpaceId && (
          <div className="enterprise-card text-center py-12">
            ...
            <p className="text-sm text-muted-foreground">Select a space to view learner progress</p>
          </div>
        )}
        ...
        {error && selectedSpaceId && (
          <div className="enterprise-card text-center py-12 text-muted-foreground">
            Failed to load learner progress data.
          </div>
        )}
        ...
        {data && (
          <div className="enterprise-card overflow-x-auto">
            ...
            <p className="text-xs text-muted-foreground mb-4">{rows.length} learner{rows.length !== 1 ? 's' : ''} enrolled</p>
            {rows.length === 0 ? (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    {['Learner', 'Enrolled', 'Last Active', 'Items Done', 'Progress', 'Completed'].map((h) => (
                  ...
                  {rows.map((r) => (
                    <tr key={r.user_id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                      ...
                      <td className="py-3 px-3 text-right tabular-nums text-muted-foreground">
                        {r.items_completed}/{r.items_total}
                      </td>
                      <td className="py-3 px-3">
                        <div className="flex items-center gap-2">
...
export default function CreatorLearnerProgressPage() {
  return <CreatorLearnerProgressReport />;
}
