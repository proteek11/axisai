...
const api = {
  ...
  fetchTeam: (id: string) =>
    fetch(`/api/teams/${id}`).then((r) => { if (!r.ok) throw new Error('Failed'); return r.json(); }),
  createTeam: (body: { name: string; description?: string }) =>
    fetch('/api/teams', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
      .then((r) => { if (!r.ok) throw new Error('Failed'); return r.json(); }),
  updateTeam: (id: string, body: Partial<{ name: string; description: string | null; is_active: boolean }>) =>
    fetch(`/api/teams/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
      .then((r) => { if (!r.ok) throw new Error('Failed'); return r.json(); }),
  deleteTeam: (id: string) =>
    fetch(`/api/teams/${id}`, { method: 'DELETE' }).then((r) => { if (!r.ok) throw new Error('Failed'); }),
  addMembers: (id: string, user_ids: string[]) =>
    fetch(`/api/teams/${id}/members`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_ids }) })
      .then((r) => { if (!r.ok) throw new Error('Failed'); return r.json(); }),
  removeMembers: (id: string, user_ids: string[]) =>
    fetch(`/api/teams/${id}/members`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_ids }) })
      .then((r) => { if (!r.ok) throw new Error('Failed'); return r.json(); }),
  fetchUsers: () =>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-background border border-border rounded-[var(--radius)] shadow-xl w-full max-w-md p-6">
        ...
        <div className="space-y-4">
          <div>
            <label className="section-label mb-1.5">Team Name *</label>
            <input
...
function DeptRow({
  ...
  allUsers: AxisUser[];
  onEdit: (d: Team) => void;
  onDelete: (d: Team) => void;
}) {
  const [open, setOpen] = useState(false);
  const [addModal, setAddModal] = useState(false);
  const qc = useQueryClient();

  const { data: detail, isLoading: loadingDetail } = useQuery<Team>({
    queryKey: ['team', dept.id],
    queryFn: () => api.fetchTeam(dept.id),
    enabled: open,
  });
  ...
  const addMut = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['team', dept.id] });
...
export function TeamsManager() {
  ...
  const [editDept, setEditDept] = useState<Team | null>(null);
  const [deleteDept, setDeleteDept] = useState<Team | null>(null);

  const { data, isLoading, isError } = useQuery<TeamListResponse>({
    queryKey: ['teams'],
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, ...body }: { id: string } & Partial<Team>) =>
      api.updateTeam(id, body),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['teams'] }); setEditDept(null); },
  });

  const deleteMut = useMutation({
    mutationFn: (id: string) => api.deleteTeam(id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['teams'] }); setDeleteDept(null); },
  });

  return (
    <div>
      <Header
        subtitle="Group users into teams for bulk Learning Space access"
        action={
          <button
      ...
      <div className="page-padding">
          ...
          <div className="enterprise-card flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-foreground">
                {teams.reduce((sum, d) => sum + d.member_count, 0)}
              </p>
        ...
        {isLoading ? (
          ...
          <div className="enterprise-card flex flex-col items-center py-12 text-center">
            <div className="w-12 h-12 rounded-full bg-red-50 flex items-center justify-center mb-3">
              <Building2 className="w-6 h-6 text-red-400" />
            </div>
            <p className="font-semibold text-foreground mb-1">Could not load teams</p>
            <p className="text-sm text-muted-foreground mb-4">
              The database may need a migration update. Run <code className="bg-muted px-1 rounded text-xs">alembic upgrade head</code> on the server, then refresh.
            </p>
            <button
              onClick={() => window.location.reload()}
              className="flex items-center gap-2 px-4 py-2 border border-[#e7e4e7] rounded-[var(--radius)] text-sm font-medium hover:bg-muted transition-colors"
            >
              Retry
            </button>
          </div>
          ...
          <div className="enterprise-card flex flex-col items-center py-16 text-center">
            ...
            <p className="font-semibold text-primary mb-1">No teams yet</p>
            <p className="text-sm text-muted-foreground mb-4">
              Create a team to group users and share Learning Spaces with the whole group.
            </p>
            <button
              onClick={() => setShowCreate(true)}
              className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-[var(--radius)] text-sm font-medium hover:bg-primary/90 transition-colors"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-background border border-border rounded-[var(--radius)] shadow-xl w-full max-w-sm p-6">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <h3 className="font-bold text-primary">Delete Team</h3>
                <p className="text-sm text-muted-foreground mt-1">
                  Are you sure you want to delete <strong>{deleteDept.name}</strong>? All
                  {deleteDept.member_count > 0 && ` ${deleteDept.member_count} membership${deleteDept.member_count !== 1 ? 's' : ''} and all`} space access grants for this team will be removed.
                </p>
              </div>
            </div>
