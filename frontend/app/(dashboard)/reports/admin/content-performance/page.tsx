...
import { Layers, Download, RefreshCw, Search } from 'lucide-react';
...
function ContentPerformanceReport() {
  ...
  const { data, isLoading, error, refetch } = useQuery<ContentPerfResponse>({
    queryKey: ['reports', 'admin', 'content-performance', dateRange],
    queryFn: () =>
      fetch(`/api/reports/admin/content-performance?date_range=${dateRange}`)
        .then((r) => r.json()),
  });
  ...
  async function handleExport() {
    const res = await fetch('/api/reports/export/csv', {
      ...
      body: JSON.stringify({ report_type: 'content_performance', filters: { date_range: dateRange } }),
    });
    if (!res.ok) return;
  ...
  return (
    <div>
      <Header
        ...
        subtitle="Views, engagement duration and completion per content item"
        action={
      ...
      <div className="p-6 space-y-4">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search content or space..."
            value={search}
          ...
          <div className="enterprise-card overflow-x-auto">
            {filtered.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-sm text-muted-foreground">No content performance data yet</p>
              </div>
            ) : (
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    {['Title', 'Type', 'Space', 'Views', 'Avg Duration', 'Completion Rate'].map((h) => (
                ...
                <tbody>
                  {filtered.map((r) => (
                    <tr key={r.content_item_id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                      <td className="py-3 px-3 font-medium max-w-[200px] truncate">{r.title}</td>
                      ...
                      <td className="py-3 px-3 text-muted-foreground max-w-[150px] truncate">{r.space_title}</td>
                      <td className="py-3 px-3 text-right tabular-nums">{r.views}</td>
